...
public class ParseHTMLToXML {
	...
	public static void Parse(String HTMLFile) {
		...
		TableTag table = null;
		try {
			table = (TableTag) parser.extractAllNodesThatMatch(tableFilter).elementAt(0);
		...
		for (int i = 1; i < rows.length; i++) {
			...
			if (action.toLowerCase().indexOf("wait") != -1) {
				...
				action = action.substring(0, action.toLowerCase().indexOf("and"));
				AddSleep = true;
			}
			...
			if (!value.equals("") && !actionNode.getName().equals("VerifyElementPresent")
					&& !actionNode.getName().equals("Click")) {
